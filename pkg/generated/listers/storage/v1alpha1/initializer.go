...
type InitializerLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.Initializer, err error)
	// Get retrieves the Initializer from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.Initializer, error)
	InitializerListerExpansion
}

// initializerLister implements the InitializerLister interface.
type initializerLister struct {
	listers.ResourceIndexer[*v1alpha1.Initializer]
}

// NewInitializerLister returns a new InitializerLister.
func NewInitializerLister(indexer cache.Indexer) InitializerLister {
	return &initializerLister{listers.New[*v1alpha1.Initializer](indexer, v1alpha1.Resource("initializer"))}
}
